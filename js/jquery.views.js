			...
			function linkSiblings(parentElem, prev, next, top) {
				...
				while (node && node !== next) {
					...
					if (isElem && (linkInfo = node.getAttribute("jsvtmpl")) || type === 8 && (linkInfo = node.nodeValue.split("jsv")[1])) {
						console.log('linkInfo:' + linkInfo.toString());
						...
						if (open) {
							...
							console.log('key:' + key.toString() + '#self:' + typeof(self) + '#selfviews:' + typeof(self.views) + '#selfviewskey:' + typeof(self.views[key]));
							
							var safetykey = '_1';
							for (var i in self.views)
							{
								console.log('thisview: ' + i.toString());
								safetykey = i;
							}
							
							//bkdebug
							if (typeof self.views[key] == 'undefined')
							{
								view = self.views[safetykey];
							}
							else
							{
								view = self.views[key];
							...
							//view = self.views[key]; //bkdebug
							//view = self.views[key] || self.views[0];
							console.log('view:' + typeof(view) + '#viewlink:' + 'h');
							if  (!view.link) {
